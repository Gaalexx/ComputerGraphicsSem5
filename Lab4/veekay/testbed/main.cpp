...
namespace
{
	...
	void initialize(VkCommandBuffer cmd)
	{
		...
		{ // NOTE: Build graphics pipeline
			vertex_shader_module = loadShaderModule("/home/gaalex/MAI/5sem/CG/Lab4/veekay/shaders/shader.vert.spv");
			...
			fragment_shader_module = loadShaderModule("/home/gaalex/MAI/5sem/CG/Lab4/veekay/shaders/shader.frag.spv");
		...
		// NOTE: Load textures and create materials
		{
			auto loadTexture = [&](const char* path) -> veekay::graphics::Texture* {
				std::vector<unsigned char> pixels;
				unsigned w, h;
				if (lodepng::decode(pixels, w, h, path) != 0) {
			...
			auto createSampler = [&](VkFilter filter, VkSamplerAddressMode mode) -> VkSampler {
				VkSamplerCreateInfo info{
					.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO,
					.magFilter = filter,
					.minFilter = filter,
			...
			auto createMaterial = [&](veekay::graphics::Texture* albedo, 
								  veekay::graphics::Texture* specular,
								  veekay::graphics::Texture* emissive,
								  VkSampler sampler) -> Material {
				Material mat;
				mat.albedo_texture = albedo ? albedo : missing_texture;
				mat.specular_texture = specular ? specular : missing_texture;
				mat.emissive_texture = emissive ? emissive : missing_texture;
				mat.sampler = sampler;
				...
				VkDescriptorImageInfo image_infos[] = {
					{.sampler = sampler, .imageView = mat.albedo_texture->view, .imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL},
					{.sampler = sampler, .imageView = mat.specular_texture->view, .imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL},
					{.sampler = sampler, .imageView = mat.emissive_texture->view, .imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL},
				};

				VkWriteDescriptorSet writes[] = {
					{.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, .dstSet = mat.descriptor_set, .dstBinding = 0, .descriptorCount = 1, .descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, .pBufferInfo = &buffer_infos[0]},
					{.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, .dstSet = mat.descriptor_set, .dstBinding = 1, .descriptorCount = 1, .descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC, .pBufferInfo = &buffer_infos[1]},
					{.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, .dstSet = mat.descriptor_set, .dstBinding = 2, .descriptorCount = 1, .descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, .pImageInfo = &image_infos[0]},
					{.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, .dstSet = mat.descriptor_set, .dstBinding = 3, .descriptorCount = 1, .descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, .pImageInfo = &image_infos[1]},
					{.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, .dstSet = mat.descriptor_set, .dstBinding = 4, .descriptorCount = 1, .descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, .pImageInfo = &image_infos[2]},
				};
				vkUpdateDescriptorSets(device, 5, writes, 0, nullptr);
				return mat;
			};

			auto dirt = loadTexture("/home/gaalex/MAI/5sem/CG/Lab4/veekay/assets/minecraft.png");
			auto woman = loadTexture("/home/gaalex/MAI/5sem/CG/Lab4/veekay/assets/chest.png");
			auto lamp = loadTexture("/home/gaalex/MAI/5sem/CG/Lab4/veekay/assets/lamp.png");
			auto grass = loadTexture("/home/gaalex/MAI/5sem/CG/Lab4/veekay/assets/grass.png");
			...
			auto white_tex = new veekay::graphics::Texture(cmd, 1, 1, VK_FORMAT_R8G8B8A8_UNORM, &white_pixel);
