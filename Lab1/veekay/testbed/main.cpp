...
std::vector<Vertex> generateTorusVertices(float majorRadius = 1.0f, float minorRadius = 0.3f, 
                                         int majorSegments = 32, int minorSegments = 16) {
	std::vector<Vertex> vertices;
	
	for (int i = 0; i <= majorSegments; ++i) { //типо мажорная окружность (сам бублик)
		float majorAngle = 2.0f * M_PI * i / majorSegments;
		float cosMajor = cosf(majorAngle);
		float sinMajor = sinf(majorAngle);
		
		for (int j = 0; j <= minorSegments; ++j) { //минорная окружность (маленькая окружность бублика)
...
std::vector<uint32_t> generateTorusIndices(int majorSegments = 32, int minorSegments = 16) {
...
void initialize() {
	...
	{ // NOTE: Build graphics pipeline
		vertex_shader_module = loadShaderModule("/home/gaalex/MAI/5sem/CG/Lab1/veekay/shaders/shader.vert.spv");
		...
		fragment_shader_module = loadShaderModule("/home/gaalex/MAI/5sem/CG/Lab1/veekay/shaders/shader.frag.spv");
	...
	auto vertices = generateTorusVertices(1.0f, 0.3f, 32, 16); 
	auto indices = generateTorusIndices(32, 16);
...
void update(double time) {
    ...
    ImGui::SliderFloat("Rotation Speed", &rotation_speed, 0.1f, 10.0f);
    ImGui::End();
...
void render(VkCommandBuffer cmd, VkFramebuffer framebuffer) {
	...
	// NOTE: ShaderConstant updates, vkCmdXXX expected to be here
	{
		...
		// NOTE: Use our torus vertex buffer
		VkDeviceSize offset = 0;
		vkCmdBindVertexBuffers(cmd, 0, 1, &vertex_buffer.buffer, &offset);

		// NOTE: Use our torus index buffer
		vkCmdBindIndexBuffer(cmd, index_buffer.buffer, offset, VK_INDEX_TYPE_UINT32);

		Vector dynamic_color = {
			(sinf(static_cast<float>(current_time)) + 1.0f) * 0.5f,
			(sinf(static_cast<float>(current_time) + 2.0f * M_PI / 3.0f) + 1.0f) * 0.5f,
			(sinf(static_cast<float>(current_time) + 4.0f * M_PI / 3.0f) + 1.0f) * 0.5f
		};
		...
		// NOTE: Draw all indices torus (33 * 17 вершин = 561 вершин, 32 * 16 * 6 индексов = 3072 индекса)
		vkCmdDrawIndexed(cmd, 32 * 16 * 6, 1, 0, 0, 0);
	}
